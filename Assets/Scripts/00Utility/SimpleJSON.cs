...
namespace SimpleJSON
{
    ...
    public class JSONNode
    {
		...
		static public explicit operator JSONNode(UnitBase u)
		{
			var node = JSON.Parse("{}");
			var h = node["typeUnit"].AsInt;
			node["typeUnit"].AsInt = (int)u.typeMe;
			node["typeAttack"].AsInt = (int)u.typeAttack;
			node["x"].AsInt = (int)u.pos.x;
			node["y"].AsInt = (int)u.pos.y;
			node["id"].AsInt = u.id;
			node["dirFacing"].AsInt = u.dirFacing;

			node["isBomb"].AsBool = u.isBomb;
			...
			if (spawn != null)
			{
				...
				node01["dirFacing"].AsInt = spawn.spawn_dirFacing;

				node01["isBomb"].AsBool = spawn.spawn_isBomb;
				node01["isSwappable"].AsBool = prefab.isSwappable;
				node01["isDestroyable_simpleAttack"].AsBool = spawn.spawn_isDestroyable_simpleAttack;
				node01["isDestroyable_bomb"].AsBool = spawn.spawn_isDestroyable_bomb;

				node.Add("other", node01); 
			}
